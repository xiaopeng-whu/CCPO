...
import numpy as np
from envs.wrappers.saute_env import saute_env	# single constraint
#from envs.wrappers.saute_env1 import saute_env	# two-constraints
...
class CustomReacherEnv(ReacherEnv):
    ...
    def reset(self) -> np.ndarray:
        ...
        #print("reset:",self.get_body_com("fingertip"))
        #print("goal:",self.get_body_com("target"))	#(1.0,1.0,0.01)
...
# single constraint SafeReacherEnv
...
"""
# two constraints SafeReacherEnv
class SafeReacherEnv(SafeEnv, CustomReacherEnv):
    safety_center = np.array([[.25, .50, 0.0]])  
    safety_center2 = np.array([[.50, 0.00, 0.0]])  
    
    def _safety_cost_fn(self, state:np.ndarray, action:np.ndarray, next_state:np.ndarray) -> np.ndarray:
        #Computes the safety cost.
        safety_vec = self.get_body_com("fingertip") - self.safety_center
        #print("body:",self.get_body_com("fingertip"))	#(0.2099,-0.005,0)
        #print("safety_vec:",safety_vec)
        dist = np.linalg.norm(safety_vec)
        #print("dist:",dist)
        safety_vec2 = self.get_body_com("fingertip") - self.safety_center2
        #print("safety_vec2:",safety_vec2)
        dist2 = np.linalg.norm(safety_vec2)
        #print("dist2:",dist2)
        cost=[0,0]
        if dist<0.4:
            #Linearly increasse from 0 to 100 based on distance 
