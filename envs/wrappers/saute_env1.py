...
def saute_env(cls):
    ...
    class SauteEnv(Env):
        ...
        def safety_step(self, cost:np.ndarray) -> np.ndarray:
            ...
            self._safety_state -= cost[0] / self.safety_budget
            self._safety_state /= self.saute_discount_factor
            self._safety_state2 -= cost[1] / self.safety_budget
            self._safety_state2 /= self.saute_discount_factor
            return self._safety_state, self._safety_state2

        def step(self, action):
            """ Step through the environment. """
            next_obs, reward, done, info = self.wrap.step(action)        
            next_safety_state, next_safety_state2 = self.safety_step(info['cost'])
            info['true_reward'] = reward         
            info['next_safety_state'] = next_safety_state
            info['next_safety_state2'] = next_safety_state2
            reward = self.reshape_reward(reward, next_safety_state, next_safety_state2)
            augmented_state  =  self._augment_state(next_obs, next_safety_state)
            augmented_state  =  self._augment_state(augmented_state, next_safety_state2)
            #print("augmented_state:",len(augmented_state))
            return augmented_state, reward, done, info

        def reshape_reward(self, reward:Array, next_safety_state:Array, next_safety_state2:Array):
            """ Reshaping the reward. """
            if self.use_reward_shaping:
                reward = reward * ((next_safety_state > 0) and (next_safety_state2 > 0)) + self.unsafe_reward * ((next_safety_state <= 0) or (next_safety_state2 <= 0)) 
            return reward
