...
def plot_data(data, xaxis='Epoch', value="AverageEpRet", 
    ...
    algo_num = int(len(data)/5)	# 5seeds
    print("algo_num:",algo_num)
    result = open('StaticPointGoal_trainingresult.txt', mode='w')
    if isinstance(data, list):
        for i in range(algo_num):
            data1 = pd.concat(data[5*i:5*(i+1)], ignore_index=True)
            #print(data1, len(data1))
            percentile_90 = data1['AverageEpCost'].quantile(0.8).round(decimals=2)
            percentile_99 = data1['AverageEpCost'].quantile(0.95).round(decimals=2)
            cost_mean = data1['AverageEpCost'].mean().round(decimals=2)
            ret_mean = data1['AverageEpRet'].mean().round(decimals=2)
            ret_std = data1['AverageEpRet'].std().round(decimals=2)
            print(data1['Condition1'][0], ":\n\tcost_mean:",cost_mean,"\tpercentile_90:",percentile_90, "\tpercentile_99:",percentile_99, "\n\treturn_mean:", ret_mean, "\treturn_std:", ret_std)
            result.writelines([str(data1['Condition1'][0]), ":\n\tcost_mean:", str(cost_mean), "\t\tpercentile_90:", str(percentile_90), "\t\tpercentile_99:", str(percentile_99), "\n\treturn_mean:", str(ret_mean), "\treturn_std:", str(ret_std), "\n"])
    ...
    for root, _, files in os.walk(logdir):
        if 'logs.txt' in files:
            ...
            try:
                ...
                continue
            performance = 'AverageTestEpCost' if 'AverageTestEpCost' in exp_data else 'AverageEpCost'
            exp_data.insert(len(exp_data.columns),'Unit',unit)
            exp_data.insert(len(exp_data.columns),'Condition1',condition1)
            exp_data.insert(len(exp_data.columns),'Condition2',condition2)
            exp_data.insert(len(exp_data.columns),'Average_EpCost',exp_data[performance])
...
def main():
    ...
    parser.add_argument('--value', '-y', default='Average_EpCost', nargs='*')
