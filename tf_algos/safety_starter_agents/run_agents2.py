...
from safe_rl.pg.buffer2 import CPOBuffer
from safe_rl.pg.network2 import count_vars, \
                               get_vars, \
                               mlp_actor_critic,\
                               placeholders, \
...
def run_polopt_agent2(train_env_fn,
                     env_name:str='',
                     log:bool=True, 
    ...
    adv_ph, cadv_ph, cadv2_ph, ret_ph, cret_ph, cret2_ph, logp_old_ph = placeholders(*(None for _ in range(7)))
    ...
    surr_cost_rescale_ph2 = tf.placeholder(tf.float32, shape=())
    cur_cost_ph2 = tf.placeholder(tf.float32, shape=())

    # Outputs from actor critic
    ac_outs = actor_critic(x_ph, a_ph, **ac_kwargs)
    pi, logp, logp_pi, pi_info, pi_info_phs, d_kl, ent, v, vc, vc2 = ac_outs

    # Organize placeholders for zipping with data from buffer on updates
    buf_phs = [x_ph, a_ph, adv_ph, cadv_ph, cadv2_ph, ret_ph, cret_ph, cret2_ph, logp_old_ph]
    ...
    if not(agent.reward_penalized):
        ...
        get_action_ops['vc2'] = vc2

    # Count variables
    var_counts = tuple(count_vars(scope) for scope in ['pi', 'vf', 'vc', 'vc2'])
    logger.log('\nNumber of parameters: \t pi: %d, \t v: %d, \t vc: %d, \t vc2: %d\n'%var_counts)
    ...
    if agent.use_penalty:
        ...
        with tf.variable_scope('penalty2'):
            # param_init = np.log(penalty_init)
            param_init2 = np.log(max(np.exp(penalty_init)-1, 1e-8))
            penalty_param2 = tf.get_variable('penalty_param2',
                                          initializer=float(param_init2),
                                          trainable=agent.learn_penalty,
                                          dtype=tf.float32)
        # penalty = tf.exp(penalty_param)
        penalty2 = tf.nn.softplus(penalty_param2)

    if agent.learn_penalty:
        if agent.penalty_param_loss:
            penalty_loss = -penalty_param * (cur_cost_ph - safety_budget)
            penalty_loss2 = -penalty_param2 * (cur_cost_ph2 - safety_budget)
        else:
            #penalty_loss = -penalty * (cur_cost_ph - safety_budget)
            penalty_loss = -penalty * (cur_cost_ph - safety_budget)
            penalty_loss2 = -penalty2 * (cur_cost_ph2 - safety_budget)
        train_penalty = MpiAdamOptimizer(learning_rate=penalty_lr).minimize(penalty_loss)
        train_penalty2 = MpiAdamOptimizer(learning_rate=penalty_lr).minimize(penalty_loss2)
    ...
    surr_cost2 = tf.reduce_mean(ratio * cadv2_ph)
    ...
    if agent.objective_penalized:
        pi_objective -= (penalty * surr_cost + penalty2 * surr_cost2)
        pi_objective /= (1 + penalty + penalty2)
    ...
    vc2_loss = tf.reduce_mean((cret2_ph - vc2)**2)
    ...
    if agent.reward_penalized:
        ...
        total_value_loss = v_loss + vc_loss + vc2_loss
    ...
    logger.setup_tf_saver(
        sess, 
        inputs={'x': x_ph}, 
        outputs={'pi': pi, 'v': v, 'vc': vc, 'vc2': vc2}
        )
    ...
    def update(epoch:int):
        if CVaR:
            cur_cost1 = logger.get_stats_cvar('EpCost1')[0]
            cur_cost2 = logger.get_stats_cvar('EpCost2')[0]
        else:
            #cur_cost1 = logger.get_stats('EpCost1')[0]
            #cur_cost2 = logger.get_stats('EpCost2')[0]
            cur_cost1 = logger.get_stats('EpCost3')[0]
            cur_cost2 = logger.get_stats('EpCost4')[0]
        c1 = cur_cost1 - safety_budget
        c2 = cur_cost2 - safety_budget
        #c = cur_cost - 30
        #print("cur_cost:", cur_cost)
        if (c1 > 0 or c2 > 0) and agent.cares_about_cost:
        ...
        inputs[cur_cost_ph] = cur_cost1
        inputs[surr_cost_rescale_ph2] = logger.get_stats('EpLen')[0]
        inputs[cur_cost_ph2] = cur_cost2
        ...
        #=====================================================================#
        if agent.learn_penalty:
            sess.run(train_penalty, feed_dict={cur_cost_ph: cur_cost1})
            sess.run(train_penalty2, feed_dict={cur_cost_ph2: cur_cost2})
    ...
    o, r, d, c1, c2, ep_ret, ep_cost1, ep_cost2, ep_len = env.reset(), 0, False, 0, 0, 0, 0, 0, 0
    ep_cost3, ep_cost4 = 0, 0
    cur_penalty = 0
    cur_penalty2 = 0
    cum_cost = 0
    cum_cost1 = 0
    ...
    for epoch in range(epochs):
        ...
        if agent.use_penalty:
            ...
            cur_penalty2 = sess.run(penalty2)
        ...
        for t in range(local_steps_per_epoch):
            ...
            vc2_t = get_action_outs.get('vc2', 0)  # Agent may not use cost value func
            ...
            if agent.reward_penalized:
                if rs:
                    if (info['next_safety_state'] > 0) and (info['next_safety_state2'] > 0):
                        r_total = r - cur_penalty*(2-info['next_safety_state']) * c1 - cur_penalty2*(2-info['next_safety_state2']) * c2
                        #r_total = r - cur_penalty*(1-math.log(info['next_safety_state'])) * c1 - cur_penalty2*(1-math.log(info['next_safety_state2'])) * c2
                        #r_total = r - cur_penalty*(float(1/(info['next_safety_state']))) * c1 - cur_penalty2*(float(1/(info['next_safety_state2']**(2)))) * c2
                    else:
                        # r_total = 0
                        r_total = -100
                    buf.store(o, a, r_total, v_t, c1, vc_t, c2, vc2_t, logp_t, pi_info_t)
            else:
                if (info['next_safety_state'] < 0 or info['next_safety_state2'] < 0):
                    # r_total = 0
                    r_total = -100	# for "Reacher2"
                else:
                    r_total = r
                buf.store(o, a, r_total, v_t, c1, vc_t, c2, vc2_t, logp_t, pi_info_t)
            logger.store(VVals=v_t, CostVVals=vc_t, CostVVals2=vc2_t)
            ...
            ep_cost2 += c2
            if rs:
                ep_cost3 += (2-info['next_safety_state']) * c1
                ep_cost4 += (2-info['next_safety_state2']) * c2
            else:
                ep_cost3 = ep_cost1
                ep_cost4 = ep_cost2
            ep_len += 1
            ...
            if terminal or (t==local_steps_per_epoch-1):
                ...
                if d and not(ep_len == max_ep_len):
                    ...
                    last_val, last_cval, last_cval2 = 0, 0, 0
                else:
                    feed_dict={x_ph: o[np.newaxis]}
                    if agent.reward_penalized:
                        ...
                        last_val, last_cval, last_cval2 = sess.run([v, vc, vc2], feed_dict=feed_dict)
                buf.finish_path(last_val, last_cval, last_cval2)

                # Only save EpRet / EpLen if trajectory finished
                if terminal:
                    if rs:
                        logger.store(EpRet=true_ep_ret, EpLen=ep_len, EpCost1=ep_cost1, EpCost2=ep_cost2, EpCost3=ep_cost3, EpCost4=ep_cost4)
                        training_rewards.extend([true_ep_ret])
                    elif saute_constraints:
                        logger.store(EpRet=true_ep_ret, EpLen=ep_len, EpCost1=ep_cost1, EpCost2=ep_cost2)
                        training_rewards.extend([true_ep_ret])
                    else:
                        logger.store(EpRet=ep_ret, EpLen=ep_len, EpCost1=ep_cost1, EpCost2=ep_cost2)
                        training_rewards.extend([ep_ret])
                    #training_costs.extend([ep_cost]) 
                    training_costs.append([ep_cost1,ep_cost2])
                ...
                pos = []
                # Reset environment
                o, r, d, c, ep_ret, ep_len, ep_cost1, ep_cost2 = env.reset(), 0, False, 0, 0, 0, 0, 0
                ep_cost3, ep_cost4 = 0, 0
                if saute_constraints:
                    true_ep_ret = 0
        #=====================================================================#
        ...
        #=====================================================================#
        cumulative_cost1 = mpi_sum(cum_cost1)
        cost_rate1 = cumulative_cost1 / ((epoch+1)*steps_per_epoch) * max_ep_len
        cumulative_cost2 = mpi_sum(cum_cost2)
        cost_rate2 = cumulative_cost2 / ((epoch+1)*steps_per_epoch) * max_ep_len
        # Save model
        if (checkpoint_frequency and (epoch % checkpoint_frequency == 0)) or (epoch == epochs-1):
            ...
            df = df.append(pd.DataFrame({
                    ...
                    "accumulated_cost1": cumulative_cost1,
                    "accumulated_cost2": cumulative_cost2,
                    "cost_rate1": cost_rate1,
                    "cost_rate2": cost_rate2,
                    "epoch": epoch,
        ...
        logger.log_tabular('CumulativeCost1', cumulative_cost1)
        logger.log_tabular('CostRate1', cost_rate1)
        logger.log_tabular('CumulativeCost2', cumulative_cost2)
        logger.log_tabular('CostRate2', cost_rate2)

        # Value function values
        logger.log_tabular('VVals', with_min_and_max=True)
        logger.log_tabular('CostVVals', with_min_and_max=True)
        logger.log_tabular('CostVVals2', with_min_and_max=True)
        ...
        logger.log_tabular('SurrCost2', average_only=True)
