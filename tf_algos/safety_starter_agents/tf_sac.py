...
def run_sac_algo(
    ...
    use_costs = fixed_cost_penalty or cost_constraint or use_mean_constraints
    polyak = 1 - tau
    ...
    # log_alpha = tf.log(alpha) # original
    ...
    if use_costs:
        ...
        # log_beta = tf.log(beta) # original
        log_beta = tf.log(tf.clip_by_value(beta, 1e-8, 1e8)) # clipping beta
    else:
    ...
    if rs:	# differ rs from lagrangian
    ...
    for t in range(total_steps // num_procs()):
        ...
        if saute_constraints and use_mean_constraints:
            if rs:
                if info['next_safety_state'] > 0:
                    r_= r - beta1*(1-math.log(info['next_safety_state'])) * c
                    #r_= r - beta1*(2-info['next_safety_state']) * c
                    #r_= r - beta1*(float(1/info['next_safety_state'])) * c
                    #print(r, r_, info['true_reward'])
                    replay_buffer.store(o, a, r_, o2, d, c)
                else:
                    replay_buffer.store(o, a, r, o2, d, c)
            else:
                replay_buffer.store(o, a, info['true_reward'], o2, d, c)
        else:
            replay_buffer.store(o, a, r, o2, d, c)
        ...
        if t > 0 and t % local_steps_per_epoch == 0:
            ...
            # test_agent(n=10) # minimal testing during training 
